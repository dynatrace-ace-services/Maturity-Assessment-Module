...
def Excel_Exporter(filename, data):
    df_writer = pd.ExcelWriter('updated_{}'.format(filename), engine = 'openpyxl', mode='a', if_sheet_exists='replace')
    data.to_excel(df_writer, sheet_name='Data', index=False)
    ws = df_writer.sheets['Data']
