...
## API Tokens:
### API v2 Scopes
	Read ActiveGate tokens, Read ActiveGates, Read analyzers, Write & execute analyzers, Read API tokens, Read attacks, Read audit logs, Read entities, Read events, Read extension monitoring configurations, Read extension environment configurations, Read extensions, Read Geographic regions, Read Hub related data, Manage metadata of Hub items, Read JavaScript mapping files, Read logs, Read metrics, Read network zones, Read OneAgents, Read problems, Read releases, Read security problems, Read settings, Read SLO, Read synthetic monitor execution results, Read synthetic locations, Look up a single trace.

### API v1 Scopes
	Capture request data, User sessions, Access problem and event feed, metrics, and topology, Change data privacy settings, Mobile symbolication file management, Create and read synthetic monitors, locations, and nodes, Read configurations, Read synthetic monitors, locations, and nodes, Fetch data from a remote environment, RUM Browser Extension, Real user monitoring JavaScripttag management, Anonymize user session data for data privacy reasons, Read credential vault entries.
...
4. Locate the template files "MaturityScript_automation_template.xlsx" and "Updated_MaturityScript_automation_template.xlsx" from the sharepoint link - https://dynatrace-my.sharepoint.com/:f:/p/shibin_nv/ErlWwac5QRpBh9MhRPgdkVEBH8LajdppDgnY8VsYal_uDw?e=aMY1mI
